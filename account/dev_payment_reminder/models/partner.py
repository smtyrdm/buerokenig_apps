...
class Partner(models.Model):
    ...
    def send_due_payment_reminder(self, date, template_id, reminder):
        if template_id:
            ...
            if invoice_ids:
                self.due_invoice_ids = [(6, 0, invoice_ids.ids)]
                email_values = {'email_from': self.env.user.company_id.email_formatted or ''}
                template_id.send_mail(self.id, force_send=True, email_values=email_values)
                invoice_ids.write({'reminder_template': reminder, 'is_reminder_sent': True})
