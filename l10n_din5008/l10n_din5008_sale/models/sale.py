...
class SaleOrder(models.Model):
    ...
    # odoo17 de exportable=False excel dışa aktarma kapatma demek, bu odoo15 de store=False
    l10n_din5008_addresses = fields.Binary(compute='_compute_l10n_din5008_addresses', store=False)
    ...
    def _compute_l10n_din5008_template_data(self):
        for record in self:
            ...
            # engin: extra/ssq_customer_sequence
            if record.partner_id.code:
                data.append((_("Kunden-Nr."), record.partner_id.code))
            # engin ensa/es_delivery
            if 'week_date' in record._fields and record.week_date:
                data.append((_("Delivery Date"), record.week_date)) # DE ve FR çevirileri eklenecek.
